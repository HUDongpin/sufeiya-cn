...
function dynamicSources(context?: LearnerContext): GroundingSource[] {
  ...
  if (context.plan) {
    ...
    sources.push({
      ...
      href: "/plan",
      sourceClass: "learner_local_record",
      content: `同轮回链的${context.plan.stage === "updated" ? "更新后" : "基础"}计划重点是 ${focus}。${time}${task}该摘要只在当前浏览器处理。`,
    });
  }
  ...
  if (context.progress) {
    ...
    sources.push({
      ...
      href: "/workspace",
      sourceClass: "learner_local_record",
      content: `打卡${progress.checkInRecorded ? "已记录" : "未记录"}；学生复盘${progress.learnerReviewConfirmed ? "已确认" : "未确认"}；微复测${progress.retestRecorded ? "已记录" : "未记录"}；更新计划${progress.updatedPlanConfirmed ? "已确认" : "未确认"}。该摘要只在当前浏览器处理。`,
    });
  }
